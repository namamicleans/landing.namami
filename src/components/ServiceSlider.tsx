                                ...
                                >
                                    <div className="p-1">
                                        <ServiceCard
                                            ...
                                            title={service.name}
                                            description={getAboutContent(service)}
                                            category={service.category.name}
                                            price={minPrice}
                                            originalPrice={basePrice > minPrice ? basePrice : undefined}
                                            duration={getDurationMinutes(service) + " min"}
                                            rating={service.rating}
                                            image={service.gallery?.[0]}
                                            isPopular={service.is_featured}
                                            features={getWhatsIncluded(service)}
                                            packages={plans.slice(0, 3).map((pkg) => ({
